...
- **年度任務管理**：新增、編輯、封存各學年度的研習任務；一鍵複製到下學年（自動順延截止日，含比對關鍵字與分眾時數規則）；負責處室採受控下拉選單（教務處／學務處／輔導室／圖書館＋未指定，v3.22 起前後端白名單驗證）
- 新增、編輯、封存所屬處室的推薦研習課程
...
| `owner` | 負責處室，受控清單（教務處／學務處／輔導室／圖書館），可留空（v3.22 起白名單驗證） |
...
- 管理者收件對象為**全校 training_admin**（v3.18 起不再依部別分流——本校為完全中學，依部別分流從根本不適用；處室分權（`task_6e2d40af`）Stage A 已完成 owner 欄位受控清單，Stage B/D 完成後將改依任務所屬處室精準路由）
