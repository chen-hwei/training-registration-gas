...
### 管理者端（需 `training_admin` 權限，v3.23 起可依 `training_scope` 收斂為處室管理者）
- **年度任務管理**：新增、編輯、封存各學年度的研習任務；一鍵複製到下學年（自動順延截止日，含比對關鍵字與分眾時數規則，**限全權管理者**）；負責處室採受控下拉選單（教務處／學務處／輔導室／圖書館＋未指定，v3.22 起前後端白名單驗證，處室管理者僅能選自己處室）
- 新增、編輯、封存所屬處室的推薦研習課程
